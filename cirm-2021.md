...
... means that genetic distance increases with geographic (straight-line) distance,
where
$$\begin{aligned}
    d(i,j)
...
$$\begin{aligned}
   \mathbb{E}[d(i, j)]
   &=
   1 - \frac{1}{\mathcal{N} + \log(\sigma_e / (\kappa \sqrt{2\mu}))}
...
The *lineage* of a bit of genome at location $x$ today
is
$$\begin{aligned}
...
1. $L$ describes *gene flow*

2. the stationary distribution of $L$ is long-term fitness

3. $\sigma_e$ is determined by $L_t$
...
Movement only happens at *reproduction*
(think about plants).
...
## Much, much simpler: in $d=1$, assume
...
- $N(t, x)$ is Markov in $x$ with states $\{G, B\}$ and transition matrix
$$\begin{aligned}
    P = \begin{bmatrix}
      1-p & p \\
      q & 1-q \\
...
- $N(t, \cdot)$ is Markov in $t$, and
- $N(t, x)$ is at stationarity, reversible with respect to the same spatial distribution as before.
...
- $\eta_t$: point measure with mass 1 for each individual
- $\epsilon$: interaction distance
- $\bar \eta_t = \eta_t / N$: rescaled, population density
- $p_\epsilon$: the heat kernel
- $\gamma(x, \bar \eta_t)$: per capita birth rate at $x$
- $q(x, y)$: probability density a juvenile disperses to $y$
- $r(y, \bar \eta_t)$: juvenile establishment probability at $y$
- $\mu(x, \bar \eta_t)$: death rate at $x$
...
1. There's still a lot we don't know about genetics + geography

2. ... especially, genetic drift!

3. But also, gene flow, which is related, and seems more tractable.
